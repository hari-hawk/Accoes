...
function MaterialIndexGridCard({
  ...
  const [approvedFileIds] = useState<Set<string>>(() => {
...
function V3OverviewContent() {
  const searchParams = useSearchParams();
  const fileParam = searchParams.get("file");
  const router = useRouter();

  // Derive project and version from mock data
  const project = mockProjects.find((p) => p.id === "proj-2")!;
  const version = mockVersions.find((v) => v.id === "ver-5")!;
  ...
  return (
    ...
    <main className="p-6 space-y-6 max-w-[1400px] mx-auto">
      ...
      <Link href="/project-v3" className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors mb-4">
          ...
          <div className="space-y-4 pt-2 min-w-0 overflow-hidden">
            {uploadFiles.length === 0 ? (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  CSV files &mdash; up to 50MB each
                </p>
